...
#         print(f"Kickoff result: {result}")  # See what's being returned
...
# Initialize session state
if 'markdown_path' not in st.session_state or not isinstance(st.session_state.markdown_path, str):
    st.session_state.markdown_path = None
if 'presentation_path' not in st.session_state or not isinstance(st.session_state.presentation_path, str):
    st.session_state.presentation_path = None

# Function to get Google services
def get_services():
    """Gets Google Slides and Drive services with token handling."""
    SCOPES = ["https://www.googleapis.com/auth/presentations", "https://www.googleapis.com/auth/drive"]
    
    creds = None
    if 'google_token' in st.session_state:
        creds = Credentials.from_authorized_user_info(json.loads(st.session_state.google_token), SCOPES)
...
# Function to copy a Google Slides template
def copy_presentation(drive_service, template_id, new_title):
    try:
        copy = drive_service.files().copy(fileId=template_id, body={'name': new_title}).execute()
        return copy.get("id")
    except HttpError as error:
        logger.error(f"Error copying template: {error}")
        raise

# Function to parse markdown into slides
def parse_markdown(file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        content = file.read()
    ...
    for i in range(0, len(slides), 3):
        slide_number = int(slides[i].strip())
        title = slides[i + 1].strip()
        content = slides[i + 2].strip()
        slide_data.append((slide_number, title, content, []))

    return sorted(slide_data, key=lambda x: x[0])

# Function to create slides in Google Slides
def create_slide(service, presentation_id, title, content, links):
    requests = [{'createSlide': {'slideLayoutReference': {'predefinedLayout': 'TITLE_AND_BODY'}}}]
    response = service.presentations().batchUpdate(presentationId=presentation_id, body={'requests': requests}).execute()
    slide_id = response['replies'][0]['createSlide']['objectId']
    ...
    requests = [{
        'insertText': {'objectId': slide_id, 'text': title}
    }, {
        'updateTextStyle': {'objectId': slide_id, 'style': {'fontSize': {'magnitude': 24, 'unit': 'PT'}}, 'fields': 'fontSize'}
    }, {
        'insertText': {'objectId': slide_id, 'text': content}
    }]

    service.presentations().batchUpdate(presentationId=presentation_id, body={'requests': requests}).execute()

# Function to export a Google Slides presentation
def export_presentation(drive_service, presentation_id, output_path):
    request = drive_service.files().export_media(
        fileId=presentation_id,
        mimeType='application/vnd.openxmlformats-officedocument.presentationml.presentation'
...
# EduFlow class with session state fixes
class EduFlow(Flow):
    def __init__(self, input_variables: Optional[Dict] = None):
        super().__init__()
        self.input_variables = input_variables or {}
    ...
    def kickoff(self) -> str:
        result = super().kickoff()
        return save_to_markdown(result, self.input_variables.get("topic", "presentation"))

# Function to save markdown content persistently
def save_to_markdown(content, topic):
    try:
        ...
        logger.error(f"Error saving markdown: {str(e)}", exc_info=True)
        raise
...
# Streamlit UI
st.title("📄 PPT Generator")
topic = st.text_input("Enter your topic", placeholder="Example: Artificial Intelligence")

if st.button("🚀 Generate Content"):
    edu_flow = EduFlow({"topic": topic})
    st.session_state.markdown_path = edu_flow.kickoff()
...
if st.session_state.markdown_path and os.path.exists(st.session_state.markdown_path):
    if st.button("🎯 Create PowerPoint Presentation"):
        ...
        with open(st.session_state.presentation_path, "rb") as file:
            st.download_button("📥 Download Presentation", data=file, file_name=os.path.basename(st.session_state.presentation_path))
